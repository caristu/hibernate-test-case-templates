	...
	static String getPropertyMappingName(Property property) {
		...
		for (Property p : property.getEntity().getProperties()) {
			if (p.getName() != null && p.getName().equalsIgnoreCase(mappingName)) {
				log.error("ERROR: Property name computation fails for property " + property + " using new name "
						+ mappingName + " there is more then one property with the same name, being robust and "
